        ...
        <header className=" flex justify-between items-center px-4 w-11/12 max-w-7xl mx-auto">
          ...
          <div className="flex-1 flex items-center justify-between">
            <div className="flex items-center gap-1">
              <MapPin />
              <span className="text-xl">{data?.current_weather.city}, {data?.current_weather.country}</span>
            </div>
        ...
        <>
          <div className="flex items-center gap-16 justify-between mt-8">
           ...
           wind_speed={data.current_weather.wind_speed}
            />
            <Map map={data?.map_html} lat={data.latitude} long={data.longitude} />
          </div>
          <Forecast data={data.forecast}/>
        </>:<div className="c-flex h-[70vh]">
          <Card className="px-10 py-5">City not found. Please try again.</Card>
          </div>}
